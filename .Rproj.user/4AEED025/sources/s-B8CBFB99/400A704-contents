...
geom_digit <- function(digits, labels) { 
        GeomRasterDigit$new(geom_params = list(digits=digits),
...
p <- ggplot(data.frame(rows=rowsToPlot, labels=labels[rowsToPlot]), aes(x=.1, y=.9, rows=rows, label=labels)) + 
